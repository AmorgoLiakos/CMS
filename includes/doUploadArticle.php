    ...
    if(in_array($fileActExt, $allowed)){
    	if($fileError===0){
    		if($fileSize<100000000){
    			...
    			$sql = "UPDATE `articles`(`title`, `content`, `artimageurl`, `authorid`, `createdate`, `categories`, `published`) VALUES ('$title','$content','$fileDestinationDb','$author','$time','$category',1)";
    			
                 $sql = "UPDATE `articles` 
                            SET `title`='$title',
                                `content`='$content',
                                `author_Fid`='$authorID',
                                `imageUrl`='$fileDestinationDb',
                                 WHERE id='{$id}'";
